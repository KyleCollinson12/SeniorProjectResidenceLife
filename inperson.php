...
<html lang="en">
  ...
  <style>
    ...
    <div class="dropdown">
      <form action="inperson.php" method="POST">
  ...
  if ($dorm != 0) {
    ...
    echo "<form action='inperson.php' method='POST'>";
...
if (strtotime($start_date) >= strtotime($current_date) || (isset($end_date) && strtotime($end_date) >= strtotime($current_date))) {
  echo "Error: You Cannot Check-In Students For This Date Because The Script Has Not Ran For This Date Yet.";
  exit();
}
...
foreach ($selected_people as $person) {
   ...
   foreach ($date_range as $date) {
       ...
       if (mysqli_num_rows($result) == 0) {
           ...
           $sql = "INSERT INTO soinprsn (StudentID, Date, InPerson, SignedOut) VALUES ('$student_id', '$datetime', '1' ,'0')";
           mysqli_query($conn, $sql);
           $inserted_row = true;
           // Update the status column in the checkintracker table if a new row was inserted
           $sql = "UPDATE checkintracker SET Status='Checked In' WHERE StudentID='$student_id' AND DATE(Date)='$date'";
           mysqli_query($conn, $sql);
       }
   }
